...
def combine():
    ...
    if dataset == 0:
        cv2.imwrite("wall-stitched.png", result)
    elif dataset == 1:
        cv2.imwrite("door-stitched.png", result)
    elif dataset == 2:
        cv2.imwrite("house-stitched.png", result)
    else:
        cv2.imwrite("pittsburgh-stitched.png", result)
...
def mousePick(x, y, idx):
    ...
    for i in range(len(pick[idx])):
        dst = cv2.circle(dst, pick[idx][i], 5, color, 2)
