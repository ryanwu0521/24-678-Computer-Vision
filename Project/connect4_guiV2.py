...
class Connect4GUI:
            ...
            for col in range(self.connect4_game.get_width()):
                ...
                if self.connect4_game.board[col][row].get_state() == 1:
                    cv2.circle(self.board_image,
                               (int((top_left[0] + bottom_right[0]) / 2), int((top_left[1] + bottom_right[1]) / 2)), int(self.square_size / 2), (0, 0, 255), -1) # Red for player 1
                elif self.connect4_game.board[col][row].get_state() == 2:
                    cv2.circle(self.board_image,
                               (int((top_left[0] + bottom_right[0]) / 2), int((top_left[1] + bottom_right[1]) / 2)), int(self.square_size / 2), (0, 255, 255), -1)  # Yellow for player 2
                else:
                    cv2.circle(self.board_image,
                               (int((top_left[0] + bottom_right[0]) / 2), int((top_left[1] + bottom_right[1]) / 2)), int(self.square_size / 2), (255, 255, 255), -1)  # White for empty
    ...
    def test_game_logic(self, col):
        ...
        # if self.connect4_game.put_chess(col):
            ...
            # Prevent message overlapping
            board_image = np.ones((self.connect4_game.get_height() * self.square_size, self.connect4_game.get_width() * self.square_size, 3), np.uint8)
            ...
            # Create new Connect4GUI with the updated board
            updated_gui = Connect4GUI(self.connect4_game)
            updated_gui.board_image = board_image
...
if __name__ == "__main__":
    ...
    # Add a small delay to allow the window to fully open
    cv2.waitKey(100)
